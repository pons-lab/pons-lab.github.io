...
- 연락처는 두 곳을 구분한다: **실험실**은 산학관 204호(+82-33-760-2859), **교수 연구실**은 백운관 201호(+82-33-760-2490).
...
뉴스·멤버·논문·강의·갤러리 같은 **콘텐츠 업데이트 요청은 짧거나 모호해도 되묻지 말고
저장소를 먼저 확인한다.** 어느 파일의 무엇을 어떻게 바꿀지 파악한 뒤,

1. **한 줄로 요약해 보여준다.** ("publications.yaml 에 논문 1편 추가 → 논문 페이지·CV·PDF 반영")
2. 사용자가 확인하면 **수정 → `npm run qa` → 커밋 → 푸시 → 배포까지 한 번에** 진행한다.
   중간에 단계마다 멈춰 묻지 않는다.
3. 배포가 끝나면 실제 URL 과 반영된 곳을 알려준다.

단, 아래 "추측 금지" 항목에 걸리는 정보가 빠져 있으면 **그 부분만 물어보고**
나머지는 그대로 진행한다. 사람 이름의 영문 표기가 없으면 추측하지 말고 반드시 묻는다.

### 추측 금지, 확인 필수 (중요)

**공개 사이트에 올라가는 사실 정보는 요청에 명시되지 않았으면 지어내지 말고 사용자에게 묻는다.**
틀린 사실이 올라가면 검증 가능한 기록이 잘못되고, 고치기 전까지 남는다.

물어봐야 하는 것:

1. **사람 이름의 영문 표기** — 로마자 표기는 본인이 논문·여권에 쓰는 것이 정답이다
2. **날짜·기간** — 합류·졸업 시기, 과제 기간, 임기
3. **과제의 역할 구분** — 연구책임자(PI)인지 참여 연구자인지
4. **논문 서지 정보** — DOI, 저널명, 권·호·페이지, IF·분위.
   **기억으로 채우지 않는다.** 사용자가 준 출처(엑셀·PDF·링크)에서만 옮긴다
5. **기관·학회·과제의 공식 영문 명칭** — 임의 번역 금지. 필요하면 공식 자료를 찾아 확인한다
6. **삭제인지 이동인지** — 파괴적 변경은 의도를 확인한다 (멤버 삭제 vs 알럼나이로 이동 등)
7. **개인정보·수치를 공개할지** — 연락처, 수상 상세, 통계 숫자

반대로 **물어보지 않고 진행하는 것** — 이미 정해진 규칙 안에서 판단하면 된다:

- 문구 다듬기, 번역 톤, 제목 문장화 (뉴스 문구 규칙을 따른다)
- 형식·정렬·줄바꿈·간격 등 표시 방식
- 디자인, 컴포넌트 구조, 파일·폴더 배치, 데이터 필드 이름
- 어느 파일을 고칠지, 어떤 순서로 작업할지

찾아보면 알 수 있는 사실(저장소 안의 데이터, 공식 문서)은 묻기 전에 **먼저 확인한다.**
확인할 수 없을 때만 묻는다.

### 멤버 온보딩·오프보딩 (중요)

멤버가 들어오고 나갈 때는 사이트 데이터와 **구글 드라이브의 Lab meeting 폴더**를
함께 챙긴다.

1. **멤버 추가** — 사이트 데이터를 고치면서 Lab meeting 아래에 그 멤버의 폴더를 만든다.
   **폴더 이름은 `members.yaml` 의 영문 이름(`name_en`)을 그대로 쓴다.**
   성까지 붙인 풀네임이다 (예: `Geonho Cheon`, `Gabriel Joseph Mlingi`).
   이미 폴더가 있으면 건너뛴다.
   다른 형식(국문 이름 등)의 폴더가 남아 있으면 임의로 바꾸지 말고, 어느 멤버 것인지
   대조해 사용자에게 보여준 뒤 이름을 바꿀지 확인받는다.
2. **알럼나이로 이동** — 멤버가 연구실을 떠나 사이트에서 알럼나이로 옮겨질 때는
   그 폴더를 `Lab meeting/Alumni/` 아래로 **옮긴다** (Alumni 폴더가 없으면 만든다).
   **절대 삭제하지 않는다.**
   폴더 안에 파일이 들어 있을 수 있으므로, 이름 변경·이동 전후로 파일 목록과 용량을
   대조해 하나도 사라지지 않았는지 확인한다.
3. 드라이브가 마운트돼 있지 않은 컴퓨터라면 이 단계는 건너뛰고 사용자에게
   **"드라이브 폴더 작업은 수동 필요"** 라고 알린다.

#### 드라이브 경로 찾기

경로는 컴퓨터마다 다르므로 **CLAUDE.md 에 적지 않는다.** 저장소의
`local.paths.json` 에 저장한다 (`.gitignore` 에 있는 컴퓨터별 파일).

1. 설정 파일의 경로가 **실제로 존재하는지 확인**하고, 유효하면 그대로 쓴다.
2. 유효하지 않으면 **모든 드라이브 문자**에서 `내 드라이브` 또는 `My Drive` 폴더를
   찾고, 그 아래 `PONS Lab/Lab meeting` 이 있는지 본다.
3. 찾으면 설정 파일을 갱신하고 사용자에게 **"경로가 ○○ 로 바뀌어 갱신했다"** 고
   알린 뒤 진행한다.
4. 못 찾으면 폴더 작업을 **멈추고 사용자에게 경로를 물어본다.**
   추측으로 새 폴더를 만들지 않는다.

### 뉴스 문구 (중요)

뉴스 제목은 신문 헤드라인이 아니라 **문장으로, 따뜻한 톤**으로 쓴다.
연구실 소식을 사람에게 전하는 말투다.

| 유형 | 틀 |
|---|---|
| 합류 | `○○ joined our lab as [직위]. Welcome!` |
| 수상 | `Congratulations to ○○ on receiving [상] at [학회]!` |
...
- **날짜와 태그를 문장에 다시 적지 않는다.** 목록에 이미 "September 2026 · People"
  이 붙으므로 제목에 "2026학년도 2학기"나 "구성원" 을 또 넣지 않는다.
- 한국어도 같은 톤으로 쓴다: "천건호·한수민 학부연구원이 연구실에 합류했습니다. 환영합니다!"
- 이름이 넷 이상이면 제목에는 인원수만 쓰고 이름은 요약문에 넣는다
  (제목이 두 줄을 넘지 않게).
- 요약문은 제목을 되풀이하지 말고 배경·의미를 덧붙인다.
- 태그: `People` / `Award` / `Grant` / `Lab` / `Research` (국문 `구성원` / `수상` /
  `과제` / `연구실` / `연구`). 새 태그가 필요하면 만들어도 된다.

### 고유명사 줄바꿈 (중요)

**주소·기관명·연구실명 같은 고유명사 구문은 의미 단위로 데이터를 나누고,
각 단위 내부에서는 줄바꿈하지 않는다.**

- 주소는 `site.yaml` 의 `contact.address.lines_en/_ko` 에 의미 단위로 적고,
  `Address.astro` 가 조각마다 `white-space: nowrap` 을 걸어 그린다.
  줄은 조각과 조각 사이에서만 바뀌고, 좁은 화면에서는 한 줄씩 쌓인다.
- 대학명·캠퍼스명·연구실명처럼 한 덩어리인 이름은 `text.js` 의 `keepTogether()`
  목록에 넣어 띄어쓰기를 줄바꿈 없는 공백으로 바꾼다.
- "Wonju-si," 뒤에서 줄이 바뀌는 것은 괜찮지만, "Wonju-si, Gangwon-" 처럼
  한 단위 가운데가 갈라지면 안 된다.
